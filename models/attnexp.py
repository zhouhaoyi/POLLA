...
from models.gcnexp import GCN_diff, GCN, GCN_cheby, GCN_com, GCN_com2, GCN_diffTest
...
class STAttentionGCNdiffLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, support_len=1, order=2, dropout=0.0, nodes=None):
        super(STAttentionGCNdiffLayer, self).__init__()
        ...
        self.gcn = GCN_diff(d_model, d_model, dropout=dropout, support_len=support_len, order=order)
    ...
    def forward(self, queries, keys, values, attn_mask, query_lengths,
        ...
        new_values = self.gcn(new_values.permute(0,3,1,2), support) # [B, D, N, T]

        new_values = new_values.permute(0,3,2,1) # [B, L, N1, D]
...
class STGCNchebyAttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, Ks=3, dropout=0.0, nodes=None):
        super(STGCNchebyAttentionLayer, self).__init__()
        ...
        self.gcn1 = GCN_cheby(d_model, d_model, Ks, dropout=dropout, nodes=nodes)
        self.gcn2 = GCN_cheby(d_model, d_model, Ks, dropout=dropout, nodes=nodes)
...
class STGCNcomAttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, support_len=1, order=2, dropout=0.0):
        super(STGCNcomAttentionLayer, self).__init__()
        ...
        self.gcn1 = GCN_com(d_model, d_model, dropout=dropout, support_len=support_len, order=order)
        self.gcn2 = GCN_com(d_model, d_model, dropout=dropout, support_len=support_len, order=order)
...
class STGCNcom2AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, dropout=0.0, nodes=None):
        super(STGCNcom2AttentionLayer, self).__init__()
        ...
        self.gcn1 = GCN_com2(d_model, d_model, dropout=dropout, nodes=nodes)
        self.gcn2 = GCN_com2(d_model, d_model, dropout=dropout, nodes=nodes)
...
class STFullAttentionTest(nn.Module):
    def __init__(self, softmax_temp=None, attention_dropout=0.0):
        super(STFullAttentionTest, self).__init__()
        self.softmax_temp = softmax_temp
        self.dropout = nn.Dropout(attention_dropout)
    ...
    def forward(self, queries, keys, values, attn_mask, query_lengths, key_lengths):
        ...
        # Compute the attention and the weighted average
        A = torch.softmax(softmax_temp * QK, dim=-1) # self.dropout(torch.softmax(softmax_temp * QK, dim=-1))
        V = torch.einsum("bnhls,bnshd->bnlhd", A, values)

        # Make sure that what we return is contiguous
        return V.contiguous(), A

class STGCNdiffAttentionLayerTest(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, support_len=1, order=2, dropout=0.0):
        super(STGCNdiffAttentionLayerTest, self).__init__()
        ...
        self.gcn1 = GCN_diffTest(d_model, d_model, dropout=dropout, support_len=support_len, order=order)
        self.gcn2 = GCN_diffTest(d_model, d_model, dropout=dropout, support_len=support_len, order=order)
    ...
    def forward(self, queries, keys, values, attn_mask, query_lengths,
        ...
        # Project the queries/keys/values
        queries = self.query_projection(queries).view(B, L, N1, H, -1)
        gcn_keys, out1 = self.gcn1(keys.transpose(-1, 1), support) # [B, D, N, T]
        keys = self.key_projection(gcn_keys.transpose(-1,1)).view(B, S, N2, H, -1) # [B, S, N, H, d]
        gcn_values, out2 = self.gcn2(values.transpose(-1, 1), support) # [B, D, N, T]
        values = self.value_projection(gcn_values.transpose(-1,1)).view(B, S, N2, H, -1) # [B, S, N, H, d]
        ...
        # Project the output and return
        return self.out_projection(new_values), [attn, out1, out2]