...
from models.attnmixhop import STMixHopAttentionLayer, STGCNAttentionLayer, STGCNdiffAttentionLayer, STLinearAttention, STFullAttention, elu_feature_map
...
class POLLA_mixhop(nn.Module):
    def __init__(self, c_in, c_out, seq_len, out_len, d_model=512, n_heads=8, n_layers=3, d_ff=8, nodes=207,
              dim_pows1=None, dim_pows2=None, dropout=0.0, activation='gelu', device=torch.device('cuda:0')):
        super(POLLA_mixhop, self).__init__()
        ...
        self.encoder = POLLAEncoder(
            [
                POLLAEncoderLayer(
                    STMixHopAttentionLayer(STLinearAttention(feature_map=elu_feature_map), d_model, n_heads, 
                                        dropout=dropout, dim_pows1=dim_pows1, dim_pows2=dim_pows2),
                    d_model,
                    d_ff,
                    dropout=dropout,
