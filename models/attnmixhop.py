...
from models.gcnmixhop import MixHopDense
...
class STMixHopAttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None, dropout=0.0, dim_pows1=None, dim_pows2=None):
        super(STMixHopAttentionLayer, self).__init__()

        # Fill d_keys and d_values
        d_keys = d_keys or (d_model//n_heads)
        d_values = d_values or (d_model//n_heads)

        dim_pows1 = dim_pows1 or [32, 64, 16]
        dim_pows2 = dim_pows2 or [64, 128, 32]
        ...
        self.gcn1 = MixHopDense(d_model, d_model, dropout=dropout, layer1_pows=dim_pows1, layer2_pows=dim_pows2)
        self.gcn2 = MixHopDense(d_model, d_model, dropout=dropout, layer1_pows=dim_pows1, layer2_pows=dim_pows2)
    ...
    def forward(self, queries, keys, values, attn_mask, query_lengths,
        ...
        gcn_keys = self.gcn1(keys, support) # [B, S, N, D]
        keys = self.key_projection(gcn_keys).view(B, S, N2, H, -1) # [B, S, N, H, d]
        gcn_values = self.gcn2(values, support) # [B, T, N, D]
        values = self.value_projection(gcn_values).view(B, S, N2, H, -1) # [B, S, N, H, d]
