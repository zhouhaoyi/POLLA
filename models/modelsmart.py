...
from models.attnsmart import FullDenseAttention, FullAttention, STDenseAttentionLayer, elu_feature_map
...
class POLLA_Dense(nn.Module):
    def __init__(self, c_in, c_out, seq_len, out_len, d_model=512, n_heads=8, n_layers=3, d_ff=8, nodes=207,
              ...
              attn_type='add'):
        super(POLLA_Dense, self).__init__()
        ...
        attn_map = {'cat': FullDenseAttention, 'add': FullDenseAttention, 'no': FullAttention}
        attn = attn_map[attn_type]
        
        self.encoder = POLLADenseEncoder(
            [
                POLLADenseEncoderLayer(
                    STDenseAttentionLayer(attn(heads=n_heads, layers=l), d_model, n_heads, 
    ...
    def forward(self, x, x_mark, attn_mask=None, attn_scores=None, length_mask=None, support=None):
        ...
        out = self.value_embedding(x) + self.temporal_embedding(x_mark) + self.position_embedding(x) + self.spatial_embedding(adp)
