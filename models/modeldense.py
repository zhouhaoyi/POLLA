...
from models.attndense import STCatAttention, STAddAttention, STAddAttention2, STAddAttention3, STGCNdiffScoreAttentionLayer, STGCNScoreAttentionLayer, \
            STGCNAttentionLayer, STGCNdiffAttentionLayer, STLinearAttention, STFullAttention, elu_feature_map
...
class POLLADenseEncoderLayer(nn.Module):
    def __init__(self, attention, d_model, d_ff=None, dropout=0.0,
                 activation="relu"):
        super(POLLADenseEncoderLayer, self).__init__()
        d_ff = d_ff or 4*d_model
        self.attention = attention
        self.linear1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=(1,1)) # nn.Linear(d_model, d_ff)
        self.linear2 = nn.Conv1d(in_channels=d_ff, out_channels=d_model, kernel_size=(1,1)) # nn.Linear(d_ff, d_model)
    ...
    def forward(self, x, attn_mask=None, attn_scores=None, length_mask=None, support=None):
        # x [B, L, N, D]
        B = x.shape[0]
        L = x.shape[1]
        N = x.shape[2]
        ...
        # Run self attention and add it to the input
        attn_scores, values = self.attention(
            x, x, x,
            attn_mask = attn_mask,
        ...
        return attn_scores, self.norm2(x+y)

class POLLADenseEncoder(nn.Module):
    def __init__(self, layers, norm_layer=None):
        super(POLLADenseEncoder, self).__init__()
        self.layers = nn.ModuleList(layers)
        self.norm = norm_layer
    ...
    def forward(self, x, attn_mask=None, attn_scores=None, length_mask=None, support=None):
        ...
        # Apply all the transformers
        for layer in self.layers:
            attn_scores, x = layer(x, attn_mask=attn_mask, attn_scores=attn_scores,
                                length_mask=length_mask, support=support)
...
class POLLA_diff_Score(nn.Module):
    def __init__(self, c_in, c_out, seq_len, out_len, d_model=512, n_heads=8, n_layers=3, d_ff=8, nodes=207,
              aptinit=None, supports=None, order=2, dropout=0.0, activation='gelu', device=torch.device('cuda:0'),
              attn_type='cat'):
        super(POLLA_diff_Score, self).__init__()
        ...
        attn_map = {'cat': STCatAttention, 'add': STAddAttention, 'add2': STAddAttention2, 'add3': STAddAttention3}
        attn = attn_map[attn_type]
        
        self.encoder = POLLADenseEncoder(
            [
                POLLADenseEncoderLayer(
                    STGCNdiffScoreAttentionLayer(attn(heads=n_heads, layers=l), d_model, n_heads, 
                                        support_len=support_len, order=order, dropout=dropout, 
                                        attn_type=attn_type),
                    d_model,
                    d_ff,
    ...
    def forward(self, x, x_mark, attn_mask=None, attn_scores=None, length_mask=None, support=None):
        # x [B, L, N, D]
        support = self.supports

        out = self.value_embedding(x) + self.temporal_embedding(x_mark) + self.position_embedding(x)# + self.spatial_embedding(se)
        out = self.embedding_dropout(out)
        out = self.encoder(out, attn_mask=attn_mask, attn_scores=attn_scores, length_mask=length_mask, support=support) # [B, L, N, D]
        out = F.relu(self.end_conv1(out)) # [B, OL, N, D]
        out = self.end_conv2(out.transpose(-1,1)).transpose(-1,1) # [B, OL, N, OD]
        
        return out # [B, L, N, D]

class POLLA_gcn_Score(nn.Module):
    def __init__(self, c_in, c_out, seq_len, out_len, d_model=512, n_heads=8, n_layers=3, d_ff=8, nodes=207,
              dropout=0.0, activation='gelu', device=torch.device('cuda:0'), attn_type='cat'):
        super(POLLA_gcn_Score, self).__init__()
        ...
        self.encoder = POLLADenseEncoder(
            [
                POLLADenseEncoderLayer(
                    STGCNScoreAttentionLayer(attn(heads=n_heads, layers=l), d_model, n_heads, 
                                        dropout=dropout, nodes=nodes, 
                                        attn_type=attn_type),
                    d_model,
                    d_ff,
