...
from models.modelT2S import T2S_trans_skip, T2S_Enc_skip, POLLA_gcn, POLLA_diff, POLLA_adpadj
...
class Exp_POLLA(Exp_Basic):
    ...
    def _build_model(self):
        model_dict = {
            ...
            'pollat2s':T2S_trans_skip,
            'pollat2sEnc':T2S_Enc_skip,
        }
        ...
        elif self.args.model=='pollat2s' or self.args.model=='pollat2sEnc':
            self.support = supports
            model = model_dict[self.args.model](
                self.args.c_in, 
                self.args.c_out, 
    ...
    def train(self, setting):
        ...
        vali_data, vali_loader = self._get_data(flag = self.args.vali)
    ...
    def test(self, setting): # args.mode=='direct'
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(test_loader):
            ...
            true = batch_y.detach().cpu().numpy().squeeze()
            pred = torch.from_numpy(pred); true = torch.from_numpy(true)
            mae = masked_mae(pred, true, 0.0).item()
            mape = masked_mape(pred, true, 0.0).item()
            rmse = masked_rmse(pred, true, 0.0).item()

            total_loss.append([mae, mape, rmse])
        total_loss = np.average(total_loss, axis=0)
        print('test mae:{}, mape:{}, rmse:{}'.format(total_loss[0], total_loss[1], total_loss[2]))
