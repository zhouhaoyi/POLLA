...
from models.modelgen import POLLA_gcn, POLLA_diff, POLLA_adpadj, \
    POLLA_gcn_full_gen, POLLA_gcn_linear_gen, POLLA_diff_full_gen, POLLA_diff_linear_gen
...
class Exp_POLLA(Exp_Basic):
    ...
    def _build_model(self):
        model_dict = {
            ...
            'polladiffLgen':POLLA_diff_linear_gen,
            'polladiffFgen':POLLA_diff_full_gen,
            'pollagcnLgen':POLLA_gcn_linear_gen,
            'pollagcnFgen':POLLA_gcn_full_gen,
        }
        supports, adjinit = self._get_adj()
        if self.args.model=='polladiff' or self.args.model=='pollaadp' \
            or self.args.model=='polladiffLgen' or self.args.model=='polladiffFgen':
            ...
            or self.args.model=='pollagcnLgen' or self.args.model=='pollagcnFgen':
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(vali_loader):
            batch_x = torch.cat([batch_x, torch.zeros_like(batch_y)], dim=1).double().to(self.device)
            batch_y = batch_y.double().to(self.device)
            batch_x_mark = torch.cat([batch_x_mark, batch_y_mark], dim=1).double().to(self.device)
